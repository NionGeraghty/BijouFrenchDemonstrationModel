...
{#if unlocked}
	<ul class="posts pt-10 pb-16">
		{#each data.course.songs as song}
			<li class="flex pb-4 flex-col md:flex-row items-start">
				...
				{#if song.mp3}
					<a
						...
						href={`https://admin.bijoufrench.boost.dev/api/songs/${song.id}/mp3`}>Song</a
					>
				{/if}
				{#if song.lyrics}
					<a
						class="mx-5 md:block whitespace-nowrap text-blue-primary hover:underline"
						href={`https://admin.bijoufrench.boost.dev/api/songs/${song.id}/lyrics`}>Lyrics</a
					>
				{/if}
			</li>
		{/each}
